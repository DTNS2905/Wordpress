...
function hc_enqueue_front_end_css() {
    ...
    if ((class_exists('woocommerce') && is_product()) || hc_get_now($HC_PAGE_ARR, array("page_setting","lightbox")) || hc_get_now($HC_PAGE_ARR, array("page_setting","popup")))  $HC_PAGE_ARR['css']['lightbox'] = "scripts/magnific-popup.css";
...
function hc_enqueue_front_end_script() {
    ...
    if ((class_exists('woocommerce') && is_product()) || hc_get_now($HC_PAGE_ARR, array("page_setting","lightbox")) || hc_get_now($HC_PAGE_ARR,array("page_setting", "popup"))) $HC_PAGE_ARR['scripts']['lightbox'] = 'jquery.magnific-popup.min.js';
...
function hc_template_meta_boxes() {
    ...
    //add_meta_box('page-settings-meta-boxes', 'Page settings', function () { echo '<a class="preview button" href="#" id="hc-page-settings" onclick="showPageSettingsBox()">' . __("Page settings","hc") . '</a><div class="clear"></div>'; }, $arr, 'side', 'low' ); //gutenberg
    add_meta_box('template-meta-boxes', 'Page components', function () { hc_block_templates_meta_boxes(); }, $arr, 'side', 'low' ); //gutenberg
}
        ...
        while ($hc_wp_query->have_posts() ) {
            ...
            if ($hc_wp_query->post->post_type == "page" || !isset($content['post_type_setting'])) {
                ...
                $item = array(
                  ...
                  "link" => get_the_permalink(),
                  "date" => get_the_date("U"),
                  "title" => $hc_wp_query->post->post_title, //gutenberg
