...
//gutenberg all page due to hc_json()

function hc_include_hc_title_tag(&$Y_NOW,$EXTRA) {
    echo "<" . esc_attr($Y_NOW["tag"]) . " " . hc_get(esc_attr($Y_NOW["id"]),'id="','"') . " class='". hc_get_component_classes($Y_NOW,$EXTRA) . "' style='" .  esc_attr($Y_NOW["custom_css_styles"]) . "'>" . wp_kses_post(hc_json($Y_NOW["text"])) . "</" . esc_attr($Y_NOW["tag"]) . ">";
}
...
<video <?php hc_echo(esc_attr($Y_NOW["id"]),'id="','" '); ?>
    <?php if (hc_true($Y_NOW['autoplay'])) echo "autoplay ";
          if (hc_true($Y_NOW['loop'])) echo "loop ";
          if (hc_true($Y_NOW['muted'])) echo "muted "; 
          if (hc_true($Y_NOW['controls'])) echo "controls "; 
...
<?php } else {
        ...
        if (hc_true($Y_NOW['autoplay'])) $att .= "&autoplay=1";
        if (hc_true($Y_NOW['loop'])) $att .= "&loop=1&playlist=" . hc_get_youtube_id($Y_NOW['link']);
        if (hc_true($Y_NOW['controls'])) $att .= "&controls=0";
?>
...
function hc_include_hc_button(&$Y_NOW,$EXTRA) {
    ...
    if (hc_true($Y_NOW["animation"]))  $css_classes .=  " anima-button ";
    if ($Y_NOW["icon"] != "") { $icon = true;  $css_classes .=  " btn-icon ";}
?>
<div <?php hc_echo(esc_attr($Y_NOW["id"]),'id="','"'); ?> class="button-cnt <?php echo hc_get_component_classes($Y_NOW,$EXTRA); ?>"
    ...
    <a
        ...
        <?php if ($icon) echo "<i class='fa " . esc_attr($Y_NOW["icon"]) . "'></i>"; echo hc_json($Y_NOW["text"]); ?>
    </a>
...
<div <?php hc_echo(esc_attr($Y_NOW["id"]),'id="','"'); ?> class="icon-list <?php echo esc_attr($Y_NOW["alignment"] . ' ' .$Y_NOW["direction"]) . ' ' . hc_get_component_classes($Y_NOW,$EXTRA); if ($icon_position == "top" || $icon_position == "bottom") echo " icon-list-top-bottom"; ?>" style="<?php  echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    ...
    for ($i = 0; $i < count($arr); $i++) {
        if ($icon_position == "right" || $icon_position == "bottom") echo '<div class="list-item"><label class="' . esc_attr($text_size) . '">'. hc_json($arr[$i]["text"]) .'</label>'. hc_get_icon($arr[$i]["icon"], $arr[$i]["icon_style"], $arr[$i]["icon_image"], $icon_size) .'</div>';
        else echo '<div class="list-item">'. hc_get_icon($arr[$i]["icon"], $arr[$i]["icon_style"], $arr[$i]["icon_image"], $icon_size) . '<label class="' . esc_attr($text_size) . '">'. hc_json($arr[$i]["text"]) .'</label></div>';
    }
    ?>
</div>
<?php }
...
<ul <?php hc_echo(esc_attr($Y_NOW["id"]),'id="','"'); ?> class="<?php echo $list_class . ' ' . (($style != "") ? 'no-icons':'') . ' ' . esc_attr($Y_NOW["icon_size"] . ' ' . $Y_NOW["alignment"]) . ' ' . hc_get_component_classes($Y_NOW,$EXTRA); ?>"
    ...
    for ($i = 0; $i < count($arr); $i++) {
        ...
        else echo '<li><i class="fa-li ' . esc_attr($arr[$i]["icon"]) . '"></i> ' . hc_json($arr[$i]["text"]) . '</li>';
    }
    ?>
</ul>
<?php }
...
<div class="icon-box text-<?php echo esc_attr($icon_position) . " " . hc_get_component_classes($Y_NOW,$EXTRA); if ($icon_position == "top" || $icon_position == "bottom") echo " icon-box-top-bottom"; if ($icon_position == "right") echo " icon-box-right"; ?>" style="<?php  echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    ...
    $text_block = '<div class="icon-box-cell"><label class="' . $title_size . '">' .  hc_json($Y_NOW["title"]) . '</label>' . ((strlen($Y_NOW["title"])> 0)?'<p class="' . $text_size . '">' . hc_json($Y_NOW["subtitle"]) . '</p></div>':'');
...
<div class="list-items <?php echo hc_get_component_classes($Y_NOW,$EXTRA) . " " . $css; ?>" style="<?php echo esc_attr($Y_NOW["custom_css_styles"]); ?>" <?php echo $anima_1 ?>>
    <?php
        $isImage = hc_true(hc_get_now($Y_NOW,"show_images"));
        $img_html = "";
        $html = "";
        for ($i = 0; $i < count($arr); $i++) {
            if ($isImage) $img_html = '<a href="' . hc_get_img($arr[$i]["image_link"], "large") . '" class="onlycover lightbox circle icon" data-lightbox-anima="fade-in" style="background-image:url(' . hc_get_img($arr[$i]["image_link"],"medium") . ')"></a>';
            $html .= '<div class="list-item' . (($isImage) ? " list-item-img":"" ) . $anima_2 . '"><div class="row"><div class="col-md-9">' . $img_html . '<h3>' . hc_json($arr[$i]["title"]) . '</h3><p>' . hc_json($arr[$i]["subtitle"]) . '</p></div><div class="col-md-3"><span>' . hc_json($arr[$i]["extra"]) . '</span></div></div></div>';
        }
        echo $html;
    ?>
</div>
...
<ul class="list-texts <?php echo hc_get_component_classes($Y_NOW,$EXTRA) . " " . $css ?>" style="<?php echo esc_attr($Y_NOW["custom_css_styles"]) ?>" <?php echo $anima_1 ?>>
    <?php
        ...
        $html = "";
        for ($i = 0; $i < count($arr); $i++) {
            $html .= ' <li' . hc_get($anima_2,' class="','"') . '><b>' . hc_json($arr[$i]["title"]) . '</b> <span>' . hc_json($arr[$i]["subtitle"]) . '</span>' . hc_get($arr[$i]["extra"],' <b>','</b>') . '</li>';
        }
        echo $html;
    ?>
</ul>
...
function hc_include_hc_counter(&$Y_NOW,$EXTRA) {
    $separator = "";
    if (isset($Y_NOW["separator"]) && $Y_NOW["separator"] != ",") {
        $separator = ' data-separator="' . $Y_NOW["separator"] . '"';
    }
    if ($Y_NOW["style"] == "") {
?>
<span class="counter-box-simple <?php echo hc_get_component_classes($Y_NOW,$EXTRA); ?>" style="<?php  echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    ...
    <?php } ?>
    <span class="counter <?php echo esc_attr(hc_get_now($Y_NOW,"text_size","text-l")); ?>" data-to="<?php echo esc_attr($Y_NOW["count_to"]); ?>" <?php hc_echo(esc_attr($Y_NOW["from"]),' data-from="','"'); hc_echo(esc_attr($Y_NOW["speed"]),' data-speed="','"'); echo $separator; hc_echo(esc_attr($Y_NOW["interval"]),' data-refresh-interval="','"'); ?>>0</span>
    <span class="<?php echo hc_get_now($Y_NOW,"text_size_2","text-s") ?>">
        <?php echo hc_json($Y_NOW["title"]); ?>
    </span>
</span>
<?php }
...
<div class="counter-box-icon icon-box <?php if ($Y_NOW["style"] == "icon") echo "icon-box-top-bottom "; echo hc_get_component_classes($Y_NOW,$EXTRA); ?>" style="<?php echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    ...
    <div class="icon-box-cell">
        ...
        <?php } ?>
        <b>
            <label class="counter <?php echo esc_attr(hc_get_now($Y_NOW,"text_size","text-l")); ?>" data-to="<?php echo esc_attr($Y_NOW["count_to"]); ?>" <?php hc_echo(esc_attr($Y_NOW["from"]),' data-from="','"'); hc_echo(esc_attr($Y_NOW["speed"]),' data-speed="','"');  echo $separator; hc_echo(esc_attr($Y_NOW["interval"]),' data-refresh-interval="','"'); ?>>0</label>
        </b>
...
function hc_include_hc_countdown(&$Y_NOW,$EXTRA) {
    ...
    if (hc_true($Y_NOW["days"])) $html .= $html_1 . '<span class="days countdown-values ' . $number_size . '">00</span>' . $html_2 . '<span class="countdown-label ' . $text_size . '"> '. hc_json($Y_NOW["days_text"]) .'</span>' . $html_3;
    if (hc_true($Y_NOW["hours"])) $html .=  $html_1 . '<span class="hours countdown-values ' . $number_size . '">00</span>' . $html_2 . '<span class="countdown-label ' . $text_size . '"> '. hc_json($Y_NOW["hours_text"]) .'</span>' . $html_3;
    if (hc_true($Y_NOW["minutes"])) $html .=  $html_1 . '<span class="minutes countdown-values ' . $number_size . '">00</span>' . $html_2 . '<span class="countdown-label ' . $text_size . '"> '. hc_json($Y_NOW["minutes_text"]) .'</span>' . $html_3;
    if (hc_true($Y_NOW["seconds"])) $html .=  $html_1 . '<span class="seconds countdown-values ' . $number_size . '">00</span>' . $html_2 . '<span class="countdown-label ' . $text_size . '"> '. hc_json($Y_NOW["seconds_text"]) .'</span>' . $html_3;
...
<div class="countdown <?php echo hc_get_component_classes($Y_NOW,$EXTRA); ?>" data-time="<?php echo esc_attr($Y_NOW["date"]); ?>" data-utc-offset="<?php echo esc_attr($Y_NOW["offset"]); ?>" style="<?php echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    <?php
        hc_echo($Y_NOW["text"] . hc_get_icon($Y_NOW["icon"], $Y_NOW["icon_style"], $Y_NOW["icon_image"], "text-xl"),'<span class="countdown-text">','</span>');
        echo $html;
        hc_echo($Y_NOW["after_text"],"<span>","</span>");
    ?>
</div>
<?php }
    if ($Y_NOW["style"] == "icon") { ?>
...
function hc_include_hc_progress_bar(&$Y_NOW,$EXTRA) {
    ...
    if ($title_pos == "top") echo '<p class="progress-label">' . hc_json($Y_NOW["title"]) . (($Y_NOW["progress_position"] == "outer") ? $counter : '') .'</p>';
    if ($title_pos == "left") echo '<div class="row vertical-row"><div class="col-md-3"><p class="progress-label">' . hc_json($Y_NOW["title"]) .
...
<?php
    if ($title_pos == "bottom") echo '<p class="progress-label">' . hc_json($Y_NOW["title"]) . (($Y_NOW["progress_position"] == "outer") ? $counter : '') .'</p>';
    if ($title_pos == "left") echo '</div></div>';
    if ($title_pos == "right") echo '</div><div class="col-md-3" style="text-align: right;"><p class="progress-label">' . hc_json($Y_NOW["title"]) .
...
function hc_include_hc_circle_progress_bar(&$Y_NOW,$EXTRA) {
    ...
    $hide = hc_true($Y_NOW["hide_percentage"]);
...
<div class="progress-circle <?php echo hc_get_component_classes($Y_NOW,$EXTRA); ?>"
    ...
    if (strlen($title) == 0 && strlen($Y_NOW["subtitle"]) == 0) {
        ...
        if ($hide && strlen($icon) == 0 && strlen($Y_NOW["subtitle"]) == 0) echo '<h2 class="main">' . $title . '</h2>';
        else { ?>
            <div class="subtitle c">
                <?php hc_echo($title,'<h2 class="main">','</h2>'); ?>
                <?php hc_echo(esc_attr($Y_NOW["subtitle"]),'<p class="sub">','</p>'); ?>
                <hr class="d" />
                <?php echo $icon; if ($hide) echo ' <span class="counter-circle"></span>'; ?>
            </div>
            <?php }
    }
            ?>
...
<ul class="timeline <?php echo hc_get_component_classes($Y_NOW,$EXTRA); ?>" style="<?php echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    ...
    <li class="<?php if ($arr[$i]["position"] == "right") echo "timeline-inverted"; ?>">
        ...
        <div class="timeline-label">
            <h4>
                <?php echo hc_json($arr[$i]["date"]); ?>
            </h4>
            <p>
                <?php echo hc_json($arr[$i]["date_subtitle"]); ?>
            </p>
        </div>
        <div class="timeline-panel">
            <div class="timeline-heading">
                <h4 class="timeline-title">
                    <?php echo hc_json($arr[$i]["title"]); ?>
                </h4>
                <p>
                    <small class="text-muted">
                        <?php echo hc_json($arr[$i]["subtitle"]); ?>
                    </small>
                </p>
            </div>
            <div class="timeline-body">
                <p>
                    <?php echo hc_json($arr[$i]["content"]); ?>
                </p>
            </div>
        </div>
    </li>
...
function hc_include_hc_social_feeds(&$Y_NOW,$EXTRA) {
    ...
    if ($type == "") {
?>
<div class="<?php echo 'social-feed-' . $source_type . ' ' . $css; if (hc_true($Y_NOW["hide_comments"])) echo " no-comments"; ?>" data-social-id="<?php echo esc_attr($Y_NOW["page_id"]); ?>" data-token="<?php echo esc_attr($Y_NOW["access_token"]); ?>" data-options="<?php echo $Y_NOW["data_options_".$source_type]; ?>"></div>
<?php }
    if ($type == "scroll_box") {
        $arr_opt = hc_get_arr_options($Y_NOW["data_sub_options_scroll_box"]);
?>
<div class="scroll-content <?php echo $css . ' '; if (hc_true(hc_get_now($arr_opt, "mobile_disabled"))) echo "scroll-mobile-disabled"; ?>" data-height="<?php echo esc_attr(hc_get_now($arr_opt,"height")); ?>" <?php hc_echo(esc_attr(hc_get_now($arr_opt, "height_remove")), 'data-height-remove="','"') ?> data-options="<?php echo $Y_NOW["data_options_scroll_box"]; ?>">
    <div class="<?php echo 'social-feed-' . $source_type; if (hc_true($Y_NOW["hide_comments"])) echo " no-comments"; ?>" data-social-id="<?php echo esc_attr($Y_NOW["page_id"]); ?>" data-token="<?php echo esc_attr($Y_NOW["access_token"]); ?>" data-options="<?php echo $Y_NOW["data_options_".$source_type]; ?>"></div>
</div>
<?php }
    if ($type == "slider" || $type == "carousel") {
?>
<div class="<?php echo 'social-feed-' . $source_type . ' ' . $css; if (hc_true($Y_NOW["hide_comments"])) echo " no-comments"; if ($type == "carousel") echo " carousel"; ?> flexslider outer-navs"
    ...
    <?php if (hc_true(hc_get_now($Y_NOW,"trigger"))) echo ' data-trigger="manual"'; ?>></div>
<?php }
}
...
<div class="btn-group social-group btn-group-icons <?php if ($Y_NOW["type"] == "classic_big") echo "btn-group-lg"; if (hc_get_now($Y_NOW,"social_colors")) echo " social-colors"; ?>" role="group">
    <?php
        if (hc_true($Y_NOW["fb"])) echo '<a class="btn btn-sm btn-border" '. get_social_link($Y_NOW["fb_link"], hc_get_now($Y_NOW,"link_type"), "facebook") .'><i class="fa fa-facebook"></i></a>';
        if (hc_true($Y_NOW["tw"])) echo '<a class="btn btn-sm btn-border" '. get_social_link($Y_NOW["tw_link"], hc_get_now($Y_NOW,"link_type"), "twitter") .'><i class="fa fa-twitter"></i></a>';
        if (hc_true($Y_NOW["g+"])) echo '<a class="btn btn-sm btn-border" '. get_social_link($Y_NOW["g+_link"], hc_get_now($Y_NOW,"link_type"), "google") .'><i class="fa fa-google-plus"></i></a>';
        if (hc_true($Y_NOW["li"])) echo '<a class="btn btn-sm btn-border" '. get_social_link($Y_NOW["li_link"], hc_get_now($Y_NOW,"link_type"), "linkedin") .'><i class="fa fa-linkedin"></i></a>';
    ?>
</div>
<?php  }
...
?>
<div class="btn-group social-group <?php if (hc_true(hc_get_now($Y_NOW,"social_colors"))) echo " social-colors"; ?>">
    <?php
        if (hc_true($Y_NOW["fb"])) echo '<a ' . get_social_link($Y_NOW["fb_link"], hc_get_now($Y_NOW,"link_type"), "facebook") . ' data-toggle="tooltip" data-placement="top" title="Facebook"><i class="fa fa-facebook ' . $class_global . '"></i></a>';
        if (hc_true($Y_NOW["tw"])) echo '<a ' . get_social_link($Y_NOW["tw_link"], hc_get_now($Y_NOW,"link_type"), "twitter") . ' data-toggle="tooltip" data-placement="top" title="Twitter"><i class="fa fa-twitter ' . $class_global . '"></i></a>';
        if (hc_true($Y_NOW["g+"])) echo '<a ' . get_social_link($Y_NOW["g+_link"], hc_get_now($Y_NOW,"link_type"), "google") . ' data-toggle="tooltip" data-placement="top" title="Google +"><i class="fa fa-google-plus ' . $class_global . '"></i></a>';
        if (hc_true($Y_NOW["li"])) echo '<a ' . get_social_link($Y_NOW["li_link"], hc_get_now($Y_NOW,"link_type"), "linkedin") . ' data-toggle="tooltip" data-placement="top" title="LinkedIn"><i class="fa fa-linkedin ' . $class_global . '"></i></a>';
    ?>
</div>
<?php  }
...
<div class="social-group-button <?php if ($Y_NOW["type"] == "button_2") echo "bottom-icons"; ?>">
    ...
    <div class="btn-group social-group">
        <?php
        if (hc_true($Y_NOW["fb"])) echo '<a ' . get_social_link($Y_NOW["fb_link"], hc_get_now($Y_NOW,"link_type"), "facebook") . ' data-toggle="tooltip" data-placement="top" title="Facebook"><i class="fa fa-facebook circle"></i></a>';
        if (hc_true($Y_NOW["tw"])) echo '<a ' . get_social_link($Y_NOW["tw_link"], hc_get_now($Y_NOW,"link_type"), "twitter") . ' data-toggle="tooltip" data-placement="top" title="Twitter"><i class="fa fa-twitter circle"></i></a>';
        if (hc_true($Y_NOW["g+"])) echo '<a ' . get_social_link($Y_NOW["g+_link"], hc_get_now($Y_NOW,"link_type"), "google") . ' data-toggle="tooltip" data-placement="top" title="Google +"><i class="fa fa-google-plus circle"></i></a>';
        if (hc_true($Y_NOW["li"])) echo '<a ' . get_social_link($Y_NOW["li_link"], hc_get_now($Y_NOW,"link_type"), "linkedin") . ' data-toggle="tooltip" data-placement="top" title="LinkedIn"><i class="fa fa-linkedin circle"></i></a>';
        ?>
    </div>
</div>
<?php  }
...
<div class="title-base <?php echo $css ?>"
    ...
    <?php hc_echo(esc_attr($Y_NOW["subtitle"]),"<p>","</p>"); ?>
    <?php if (hc_true($Y_NOW["scroll_top"])) echo '<i class="fa fa-angle-up scroll-top"></i>'; ?>
</div>
<?php }
...
<p class="block-quote <?php echo hc_get_component_classes($Y_NOW,$EXTRA); if ($Y_NOW["style"] == "") echo " quote-1"; if ($Y_NOW["style"] == "double_quote") echo " quote-2"; ?> " style="<?php  echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    <?php
    echo hc_json($Y_NOW["text"]);
    hc_echo(hc_json($Y_NOW["author"]),'<span class="quote-author">','</span>');
    ?>
</p>
<?php }
...
<table class="table table-hover bootgrid-table <?php echo esc_attr($Y_NOW["style"]) . ' ' . hc_get_component_classes($Y_NOW,$EXTRA); ?>" data-options="<?php echo esc_attr($Y_NOW["data_options"]);  ?>" style="<?php  echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    <thead>
        ...
        for ($i=1; $i <= $col_num; $i++) {
            ...
            echo "<th data-column-id='table_c_" . $i ."'" . $format  . ">" . hc_json($val) . "</th>";
        };
            ?>
        </tr>
    </thead>
    <tbody>
        <?php for ($i=0; $i < count($Y_NOW["table"]["rows"]); $i++) {
                  echo "<tr>";
                  for ($j=1; $j <= $col_num; $j++) echo "<td data-column-id='table_c_" . $i ."'>" . hc_json($Y_NOW["table"]["rows"][$i]["c" . $col_num . "_" . $j . "_content"]) . "</td>";
                  echo "</tr>";
              } ?>
    </tbody>
</table>
...
<table class="table table-hover <?php echo esc_attr($Y_NOW["style"] . ' ' . hc_get_component_classes($Y_NOW,$EXTRA)); ?>" style="<?php  echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    <thead>
        <tr>
            <?php for ($i=1; $i <= $col_num; $i++) echo "<th>" . hc_json($Y_NOW["table"]["c" . $col_num . "_" . $i . "_header"]) . "</th>"; ?>
        </tr>
    </thead>
    <tbody>
        <?php for ($i=0; $i < count($Y_NOW["table"]["rows"]); $i++) {
                  echo "<tr>";
                  for ($j=1; $j <= $col_num; $j++) echo "<td>" . hc_json($Y_NOW["table"]["rows"][$i]["c" . $col_num . "_" . $j . "_content"]) . "</td>";
                  echo "</tr>";
              } ?>
    </tbody>
</table>
...
function hc_include_hc_inner_menu(&$Y_NOW,$EXTRA) {
    ...
    $one_page = hc_true($Y_NOW["one_page"]);
    ...
    for ($i = 0; $i < $count; $i++) {
        if (!$one_page) $link = esc_url($menu_items[$i]["link"]); else $link = "#" . $arr_sections_id[$i];
        if (!hc_true($menu_items[$i]["child"])) {
            if ($i + 1 < $count && hc_true($menu_items[$i + 1]["child"])) {
                if ($menu_type == "horizontal") $menu_html .= '<li class="dropdown"><a class="dropdown-toggle" data-toggle="dropdown" href="#">' . hc_get_icon($menu_items[$i]["icon"],"","","") . hc_json($menu_items[$i]["text"]) . '<i class="caret"></i></a>';
                if ($menu_type == "vertical") $menu_html .= '<li><a class="dropdown" data-toggle="dropdown" href="#">' . hc_get_icon($menu_items[$i]["icon"],"","","") . hc_json($menu_items[$i]["text"]) . ' <span class="fa arrow"></span></a>';
                ...
                while ($j < $count && $menu_items[$j]["child"] == "true") {
                    ...
                    $html .= '<li><a ' . (($Y_NOW["one_page"] != "true") ? 'onclick="document.location=\'' . $sub_link . '\'"':"") . ' href="' . $sub_link . '">'. hc_get_icon($menu_items[$j]["icon"],"","","") . hc_json($menu_items[$j]["text"]) . '</a></li>';
                    $j++;
                }
                $i = $j-1;
                $menu_html .=  $html . '</ul></li>';
            } else $menu_html .= '<li><a href="' . $link . '">'. hc_get_icon($menu_items[$i]["icon"],"","","") . hc_json($menu_items[$i]["text"]) . '</a></li>';
        }
    }
    $menu_html .= '</ul>';
...
function hc_include_hc_contact_form(&$Y_NOW,$EXTRA) {
    $css = hc_get_component_classes($Y_NOW,$EXTRA);
    if (hc_true($Y_NOW["label"])) $css .= " label-visible";
...
<form action="<?php echo HC_PLUGIN_URL ?>/scripts/php/contact-form.php" class="form-box form-ajax form-ajax-wp <?php echo $css ?>" method="post" data-email="<?php echo $Y_NOW["recipient_email"]; ?>" <?php hc_echo(hc_get_now($Y_NOW,"subject"),'data-subject="','"') ?> <?php hc_echo(esc_attr($Y_NOW["custom_css_styles"]),'style="','"'); ?>>
    ...
    if ($Y_NOW["button_style"] != "link") {
        ...
        if (hc_true(hc_get_now($Y_NOW,"button_animation"))) $button .= " anima-button ";
    }
    if ($Y_NOW["button_position"] == "inline") $button .=  " pull-right ";
    if ($Y_NOW["button_icon"] != "") { $icon = true;  $button .=  " btn-icon ";}
    $button = '<button class="' . $button . '" type="submit">' . (($icon) ? "<i class='" . $Y_NOW["button_icon"] . "'></i>":"") . $button_text . '</button>';

    $arr = $Y_NOW["rows"];
    for ($i = 0; $i < count($arr); $i++) {
        $text = hc_json($arr[$i]["text"]);
        $id = str_replace(" ","-",$text);
