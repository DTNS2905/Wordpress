...
<div data-height-remove="<?php echo esc_attr($Y_NOW["remove_height"]); ?>" class="scroll-content <?php echo hc_get_component_classes($Y_NOW,$EXTRA); if (hc_true($Y_NOW["mobile_disabled"])) echo " scroll-mobile-disabled"; ?>" data-height="<?php if (hc_get_now($Y_NOW,"full_screen")) echo "fullscreen"; else echo esc_attr($Y_NOW["height"]); ?>" data-options="<?php echo esc_attr($Y_NOW["data_options"]); ?>" style="<?php  echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    <?php
...
function hc_include_hc_grid_table(&$Y_NOW,$EXTRA) {
    ...
    $css = "";
    if (hc_true(hc_get_now($Y_NOW,"full-border"))) {
        $css = " full-border-table";
    }
    if (!hc_true(hc_get_now($Y_NOW,"no-borders"))) {
        $css .= " border-table";
    }
    if (hc_true(hc_get_now($Y_NOW,"full-mobile"))) {
...
function hc_include_hc_image_slider(&$Y_NOW,$EXTRA) {
    ...
    if (hc_true(hc_get_now($Y_NOW,"nav_inner"))) $cnt_classes .= " nav-inner";
    if (hc_true($Y_NOW["outer_arrows"])) $cnt_classes .= " outer-navs";
    if (hc_true($Y_NOW["visible_arrows"])) $cnt_classes .= " visible-dir-nav";
    if (hc_true(hc_get_now($Y_NOW,"png-over"))) $cnt_classes .= " png-over";
?>
<div <?php hc_echo(esc_attr($Y_NOW["id"]),'id="','"'); ?> class="flexslider <?php echo $cnt_classes ?>"
    ...
    if (hc_true(hc_get_now($Y_NOW,"thumbnail"))) {
...
<div <?php hc_echo(esc_attr($Y_NOW["id"]),'id="','"'); ?> class="section-slider <?php echo hc_get_component_classes($Y_NOW,$EXTRA); ?>">
    <div class="flexslider advanced-slider slider <?php if (hc_true(hc_get_now($Y_NOW,"nav_inner"))) echo " nav-inner"; if (hc_true($Y_NOW["outer_arrows"])) echo " outer-navs"; if (hc_true($Y_NOW["visible_arrows"])) echo " visible-dir-nav"; ?>"
        data-options="<?php echo esc_attr($Y_NOW["data_options"]) ?>" style="<?php echo esc_attr($Y_NOW["custom_css_styles"]); ?>" <?php hc_echo(hc_get_now($Y_NOW,"trigger"),"data-trigger='","'") ?>>
        <ul class="slides">
            <?php
...
function hc_tab_menu(&$Y_NOW,$classes) {
    $style = $Y_NOW["style"];
?>
<ul class="nav <?php if ($style=="justified" || $style=="justified-bottom") echo "nav-justified "; if ($style == "vertical-left-justified" || $style == "vertical-right-justified") echo "nav-justified-v "; if ($style=="centered" || $style=="centered-bottom") echo "nav-center "; echo esc_attr($classes); if (hc_true($Y_NOW["pills"])) echo " nav-pills"; else echo " nav-tabs"; ?>">
    <?php
    for ($i = 1; $i < 9; $i++) {
        if (isset($Y_NOW["content_" . $i]) && count($Y_NOW["content_" . $i]) > 0) echo "<li" . (($i==1) ? " class='active'" : "") . hc_get(esc_attr($Y_NOW["animation_icon"])," data-anima='","' data-trigger='hover'") ."><a>" . hc_get_icon($Y_NOW["icon_" . $i],$Y_NOW["icon_style_" . $i],$Y_NOW["icon_image_" . $i], ((strlen($Y_NOW["animation_icon"]) > 0) ? "anima ":"")) . " " . $Y_NOW["name_" . $i] . "</a></li>";
    }
    ?>
</ul>
<?php   }
...
?>
<div class="tab-box <?php echo esc_attr($tab_style) . ' ' .  hc_get_component_classes($Y_NOW,$EXTRA); if (hc_true($Y_NOW["pills"])) echo " pills"; ?>" <?php hc_echo(esc_attr($Y_NOW["animation"]),"data-tab-anima='","'"); ?> style="<?php echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    <?php
        $col_menu = "col-md-4";
        $col_panel = "col-md-8";
...
?>
<div class="tab-box <?php echo hc_get_component_classes($Y_NOW,$EXTRA) . ' '; if ($tab_style == "bottom") echo "inverse"; if (hc_true($Y_NOW["pills"])) echo " pills"; ?>" <?php hc_echo(esc_attr($Y_NOW["animation"]),"data-tab-anima='","'"); ?> style="<?php echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    <?php
        if ($tab_style == "bottom") {
            hc_tab_content($Y_NOW,$EXTRA);
            hc_tab_menu($Y_NOW,"");
...
<div class="collapse-box <?php echo hc_get_component_classes($Y_NOW,$EXTRA); ?>" <?php hc_echo(esc_attr($Y_NOW["height"]),"data-height='","'"); ?> style="<?php  echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    <?php if ($Y_NOW["position"] == "top") echo '<div class="text-' . esc_attr(hc_get_now($Y_NOW,"alignment","center")) .'"><a class="btn btn-s collapse-button" data-button-open-text="' . hc_get_now($Y_NOW,"button_open_text","Close") . '"'. hc_get(esc_attr($Y_NOW["time"])," data-time='","'") .'><b>' . hc_get_now($Y_NOW,"button_text","Show all") . '</b><span class="caret"></span></a></div>'; ?>
    <div class="panel">
        <?php
    $arr = $Y_NOW["content"];
    $CURRENT_SECTION = $Y_NOW["content"];
    ...
    <?php if ($Y_NOW["position"] == "bottom") echo '<div class="text-' . hc_get_now($Y_NOW,"alignment","center") .'"><a class="btn btn-sm collapse-button" data-button-open-text="' . hc_get_now($Y_NOW,"button_open_text","Close") . '" ' . hc_get(esc_attr($Y_NOW["time"])," data-time='","'") .'><b>' . hc_get_now($Y_NOW,"button_text","Show all") . '</b><span class="caret"></span></a></div>'; ?>
</div>
<?php }
...
?>
<div class="grid-list <?php echo hc_get_component_classes($Y_NOW,$EXTRA); if (hc_true(hc_get_now($Y_NOW,"gallery")) || $Y_NOW["tab_index"] == 0) echo " gallery"; if($Y_NOW["column"] == "col-md-12") echo " one-row-list"; ?>" style="<?php  echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    ...
    <?php if (hc_true($Y_NOW["pagination"])) {
              if (hc_true($Y_NOW["pag_centered"])) echo "<div class='list-nav'>"; ?>
    <ul class="<?php echo esc_attr($Y_NOW["pag_size"]); if (hc_true($Y_NOW["pag_hide_controls"])) echo " hide-first-last"; ?> pagination-grid" data-page-items="<?php if (strlen($Y_NOW["pag_items"]) > 0 ) echo esc_attr($Y_NOW["pag_items"]); else echo "3"; ?>" <?php hc_echo(esc_attr($Y_NOW["pag_animation"]),"data-pagination-anima='","'"); ?> data-options="<?php echo $Y_NOW["pag_data_options"]; ?>"></ul>
    <?php if (hc_true($Y_NOW["pag_centered"])) echo "</div>";
          }
          if (hc_true($Y_NOW["load_more"])) {  ?>
    <div class="list-nav">
        <a class="btn-border btn btn-sm load-more-grid" <?php hc_echo(esc_attr($Y_NOW["lm_animation"]),"data-pagination-anima='","'"); ?>
...
function hc_include_hc_masonry_list(&$Y_NOW,$EXTRA = array()) {
    ...
    $gallery = false;
    $css = "";
    if (hc_true(hc_get_now($Y_NOW,"gallery")) || $Y_NOW["tab_index"] == 0) {
        $gallery = true;
        $css .= " gallery";
    }
    if ($Y_NOW["column"] == "col-md-12") $css .= " one-row-list";
    if (hc_true($Y_NOW["auto_masonry"]))  $css .= " maso-layout";
...
<div class="maso-list <?php echo hc_get_component_classes($Y_NOW,$EXTRA); echo $css; ?>"
    ...
    <div class="navbar navbar-inner">
        ...
        <div class="collapse navbar-collapse <?php if ($Y_NOW["menu_position"] == "nav-center") echo "text-center" ?>">
            <ul class="nav navbar-nav over inner maso-filters <?php echo esc_attr($Y_NOW["menu_style"] . " " . $Y_NOW["menu_position"]); ?>">
                <li class="current-active active">
                    <a class="maso-filter-auto" data-filter="maso-item">
                        <?php echo hc_get_now($Y_NOW,"all_text","All") ?>
                    </a>
                </li>
                <?php
              ...
              if ($Y_NOW["tab_index"] == 1) {
                  for ($i = 1; $i < 11; $i++)  {
                      if (isset($Y_NOW["content_" . $i]) && count($Y_NOW["content_2_" . $i]) > 0) echo '<li><a data-filter="cat-'. esc_attr($i) .'">'. esc_attr($Y_NOW["name_2_" . $i]) .'</a></li>';
                  }
              }
                ?>
                <li>
    ...
    <div class="maso-box row" <?php hc_echo(esc_attr($Y_NOW["lightbox_animation"]),"data-lightbox-anima='","'"); if (strlen($Y_NOW["margins"]) > 0) echo ' style="margin: -'. esc_attr($Y_NOW["margins"]) . 'px; width: calc(100% + ' . intval ($Y_NOW["margins"]) * 2 . 'px"'; ?>>
        ...
        <?php             }
              ...
              if ($Y_NOW["tab_index"] == 1) {
                  for ($j = 1; $j < 11; $j++)  {
                      if (isset($Y_NOW["content_2_" . $j]) && count($Y_NOW["content_2_" . $j]) > 0) {
    ...
    <?php if (hc_true($Y_NOW["pag_centered"])) echo "</div>";
          ...
          if (hc_true($Y_NOW["load_more"])) {  ?>
    <div class="list-nav">
        <a class="btn-border btn btn-sm load-more-maso" <?php hc_echo(esc_attr($Y_NOW["lm_animation"]),"data-pagination-anima='","'"); ?>
            data-page-items="<?php if (strlen($Y_NOW["lm_items"]) > 0 ) echo esc_attr($Y_NOW["lm_items"]); else echo "3"; ?>"
            data-options="<?php if (strlen($Y_NOW["lm_lazy"]) > 0 ) echo "lazyLoad:true"; ?>">
            <?php echo esc_attr($Y_NOW["lm_button_text"]); ?>
            <i class="fa fa-arrow-down"></i>
...
function hc_include_hc_background_icon_box(&$Y_NOW,$EXTRA) {
    //This item is deprecated and will be remove soon.
} ?>
