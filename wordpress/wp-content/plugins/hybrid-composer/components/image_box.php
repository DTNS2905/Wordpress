...
function hc_include_hc_image_box(&$Y_NOW,$EXTRA) {
    ...
    $text = hc_json($Y_NOW['caption_img_box']);
    ...
    if ($text != ""  && hc_true($Y_NOW['img_box_inner_caption'])) $css_classes .= "inner ";
...
<a id="<?php echo esc_attr($Y_NOW["id"]); ?>" <?php hc_set_link_settings($Y_NOW, $css_classes);
                                                    ...
                                                    if (hc_true($Y_NOW['icon_hidden'])) echo ' data-anima-out="hide"';
    ...
    if ($text != "") {
        if ($img_style != "circle" && $img_style != "circle_thumbnail") {
            if (hc_true($Y_NOW['img_box_inner_caption'])) echo '<span class="caption-box"><span class="caption">' . strip_tags($Y_NOW['caption_img_box'],'<b><span><br>') . '</span></span></a>';
            else echo '<span class="caption">'. esc_attr($text) .'</span></a>';
        } else {
            if (hc_true($Y_NOW['img_box_inner_caption'])) echo '<span class="caption">'. strip_tags($text,'<b><span><br>') .'</span></a>';
...
function hc_include_hc_adv_image_box(&$Y_NOW,$EXTRA) {
    ...
    if ($Y_NOW["button_text"] != "") {
        ...
        $button = '<hr class="space s"><a ' . hc_get_link_settings($Y_NOW, $button)  . '>' . ((hc_get_now($Y_NOW,"button_animation")) ? '<i class="fa fa-long-arrow-right"></i> ':'') . hc_json($Y_NOW["button_text"]) . '</a>';
    }

    if ($style == "double_content") { ?>
    <div <?php hc_echo(esc_attr($Y_NOW["id"]),'id="','"'); ?> class="adv-img-double-content">
        <div class="img-box adv-img adv-img-half-content <?php echo esc_attr($img_anima_no_hidden . ' ' . $css_cnt); ?>" <?php if (hc_true($Y_NOW["hidden_content"])) echo 'data-anima="fade-bottom" data-trigger="hover" data-anima-out="hide"'; ?> style="<?php  echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
            ...
            <div class="caption <?php if (hc_true($Y_NOW["hidden_content"])) echo "anima" ?>">
                <?php hc_echo(esc_attr(hc_get_now($Y_NOW,"extra_text")),"<h3>","</h3>"); ?>
                <p>
                    <?php echo hc_json($Y_NOW["text"]) ?>
                </p>
                <?php echo $button; ?>
            </div>
    ...
    <div <?php hc_echo(esc_attr($Y_NOW["id"]),'id="','"'); ?> class="img-box adv-img adv-img-side-content i-top-right <?php echo esc_attr($img_anima_no_hidden . ' ' . $css_cnt); ?>" <?php if (hc_true($Y_NOW["hidden_content"])) echo 'data-anima="fade-left" data-trigger="hover" data-anima-out="hide"' ?> style="<?php  echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    ...
    <div <?php hc_echo(esc_attr($Y_NOW["id"]),'id="','"'); ?> class="img-box adv-img adv-img-button-content <?php echo esc_attr($css_cnt); ?>" data-anima="fade-left" data-trigger="hover" <?php if (hc_true($Y_NOW["hidden_content"])) echo 'data-anima-out="hide"' ?> style="<?php  echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
        <a <?php echo ' class="' . esc_attr($css_classes) . '" href="#"' ?>>
            <img alt="" src="<?php echo esc_url($img)?>">
        </a>
        <div class="caption <?php if (hc_true($Y_NOW["hidden_content"])) echo "anima anima-fade-top" ?>">
            <div class="inner">
                <?php hc_echo(esc_attr($Y_NOW["title"]),"<h2>","</h2>"); ?>
                <?php hc_echo(esc_attr($Y_NOW["subtitle"]),'<p class="sub">','</p>'); ?>
                <?php hc_echo($Y_NOW["text"],'<p>','</p>'); ?>
    ...
    <a <?php hc_echo(esc_attr($Y_NOW["id"]),'id="','"'); ?> <?php hc_set_link_settings($Y_NOW, "img-box adv-circle circle adv-circle-center ". esc_attr($css_classes) . (($Y_NOW["hidden_content"] == "true") ? "":" show")); ?> 
        <?php if (hc_true($Y_NOW["hidden_content"])) { 
       ...
       <?php if (hc_true($Y_NOW["hidden_content"])) { 
        ...
        <div class="caption <?php echo esc_attr($anima_css) ?>">
            <div class="inner">
                ...
                <?php hc_echo(esc_attr(hc_get_now($Y_NOW,"extra_text")),"<span class='extra-content'>","</span>"); ?>
                <?php hc_echo(esc_attr($Y_NOW["subtitle"]),'<p>','</p>'); ?>
                <?php hc_echo($Y_NOW["text"],'<p class="sub">','</p>'); ?>
            </div>
        </div>
        <?php hc_set_content_lightbox($Y_NOW); ?>
    </a>
    ...
    <a <?php hc_echo(esc_attr($Y_NOW["id"]),'id="','"'); ?> <?php hc_set_link_settings($Y_NOW, "img-box adv-circle circle " . esc_attr($css_classes) . (($style == "circle_bottom") ? " adv-circle-bottom":" adv-circle-half") . (($Y_NOW["hidden_content"] == "true") ? "":" show"));  
             if (hc_true($Y_NOW['hidden_content'])) echo ' data-anima="fade-bottom" data-trigger="hover" data-anima-out="hide"'; ?>
        style="<?php echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
    ...
    <a <?php hc_echo(esc_attr($Y_NOW["id"]),'id="','"'); ?> <?php hc_set_link_settings($Y_NOW, "img-box adv-circle circle adv-circle-bg " . esc_attr($css_classes) . (($Y_NOW["hidden_content"] == "true") ? "":" show")); 
             if (hc_true($Y_NOW['hidden_content'])) echo ' data-anima="' . ((strlen($Y_NOW["image_animation"]) > 0) ? $Y_NOW["image_animation"]:"show-scale") .'" data-trigger="hover" data-anima-out="hide"'; ?> 
        style="<?php  echo esc_attr($Y_NOW["custom_css_styles"]); ?>">
