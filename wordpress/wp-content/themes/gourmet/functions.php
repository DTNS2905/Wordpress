...
function wptf_register_required_plugins() {
	$plugins = array(
        array(
            'name'               => esc_html__('Slider Revolution',"gourmet"),
            'slug'               => 'revslider',
        ...
        array(
			'name'               => esc_html__('Hybrid Composer',"gourmet"),
...
add_action( 'tgmpa_register', 'wptf_register_required_plugins' );
if (!isset( $content_width )) $content_width = 1200;
add_theme_support( 'automatic-feed-links' );
add_theme_support('title-tag');
function wptf_enqueue_front_end_script() {
...
add_action('wp_enqueue_scripts', 'wptf_enqueue_front_end_script');
...
<select data-hc-setting="sidebars" id="sidebars-menu" name="sidebars-menu">
    <option value="" <?php if ($sidebar == "") echo "selected" ?>><?php esc_attr_e("None","gourmet") ?></option>
    <option value="right" <?php if ($sidebar == "right") echo "selected" ?>><?php esc_attr_e("Right","gourmet") ?></option>
    <option value="left" <?php if ($sidebar == "left") echo "selected" ?>><?php esc_attr_e("Left","gourmet") ?></option>
    <option value="right-left" <?php if ($sidebar == "right-left") echo "selected" ?>><?php esc_attr_e("Right and left","gourmet") ?></option>
</select>
<?php
...
add_action('add_meta_boxes', 'wptf_sidebar');

function wptf_theme_add_editor_styles() {
    add_editor_style(WPTF_URL . "/css/tiny-mce.css");
}
add_action('admin_init', 'wptf_theme_add_editor_styles');

//MENU
function wptf_init_menus() {
    register_nav_menus(
          array(
            'header-menu' => esc_html__('Header Menu',"gourmet"),
            'footer-menu' => esc_html__('Footer Menu',"gourmet"),
            'extra-menu' => esc_html__('Top mini Menu',"gourmet")
          )
    );
}
add_action('after_setup_theme', 'wptf_init_menus');

//WIDGETS
function wptf_widgets_init() {
    register_sidebar( array(
        'name'          => esc_html__('Right Sidebar',"gourmet"),
        'id'            => 'right_side_bar',
        'description'   => esc_html__('Global sidebar for pages, enable it on single page.',"gourmet"),
    ...
    register_sidebar(array(
        'name'          => esc_html__('Left Sidebar',"gourmet"),
        'id'            => 'left_side_bar',
        'description'   => esc_html__('Global sidebar for pages, enable it on single page.',"gourmet"),
        'before_widget' => '<div class="list-group list-blog">',
        'after_widget'  => '</div>',
    ...
    if (class_exists('WooCommerce')) {
        register_sidebar(array(
           'name'          => esc_html__('Woocommerce Shop Sidebar Left',"gourmet"),
           'id'            => 'woocommerce_shop_left_side_bar',
           'description'   => esc_html__('Shop sidebar, enable it on Theme options > List Post Types',"gourmet"),
           'before_widget' => '<div class="list-group list-blog">',
           'after_widget'  => '</div>',
           'before_title'  => '<p class="list-group-item active">',
           'after_title'   => '</p>',
        ...
        register_sidebar(array(
          'name'          => esc_html__('Woocommerce Shop Sidebar Right',"gourmet"),
        ...
        register_sidebar(array(
          'name'          => esc_html__('Woocommerce Item Sidebar Left',"gourmet"),
          'id'            => 'woocommerce_single_left_side_bar',
          'description'   => esc_html__('Single product sidebar, enable it on Theme options > List Post Types',"gourmet"),
          'before_widget' => '<div class="list-group list-blog">',
          'after_widget'  => '</div>',
          'before_title'  => '<p class="list-group-item active">',
          'after_title'   => '</p>',
        ...
        register_sidebar(array(
           'name'          => esc_html__('Woocommerce Item Sidebar Right',"gourmet"),
...
add_action( 'widgets_init', 'wptf_widgets_init' );

//MAIN CONTENT
function wptf_get_post_info() {
    $txt = "";
    ...
    if ( ! empty( $categories ) ) {
        foreach( $categories as $category ) {
            $output .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" alt="' . esc_attr( sprintf( __("View all posts in %s","gourmet"), $category->name ) ) . '">' . esc_html( $category->name ) . '</a>' . $separator;
        }
    ...
    <div class="header-base search-results-header">
        <div class="container">
            <div class="row">
                <div class="col-md-9">
                    <div class="title-base text-left">
                        <h1><?php echo esc_html__("Search results for ","gourmet") . get_search_query() ?></h1>
                        <p><?php esc_attr_e("Search results for ","gourmet"); echo '<b><i>' . get_search_query() . '</i></b>'; ?></p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="search-page-box">
                    <form role="search" method="get" id="searchform" class="navbar-form" action="<?php echo esc_url(home_url()) ?>">
                        <div class="input-group">
                            <input name="s" id="s" type="text" class="form-control" placeholder="<?php esc_attr_e("Search for ...","gourmet"); ?>" />
                            <span class="input-group-btn">
                                <input class="btn btn-default" type="submit" id="searchsubmit" value="<?php esc_attr_e("Go","gourmet"); ?>" />
                            </span>
                        </div>
                    </form>
                </div>
...
?>                         <h3>
            ...
            if (defined("HC_PLUGIN_PATH")) {
                echo hc_get_the_excerpt(get_the_excerpt());
                            ...
                            <a class="circle-button btn btn-sm" href="<?php echo esc_url($link) ?>">
                                <?php esc_attr_e("Read more","gourmet") ?>
                            </a>
                        </div>
                    </div>
                </div>
    ...
    } else echo "<h2 class='no-search-results'>" . esc_html__("No results found ...","gourmet") . "</h2><hr class='space row-500' />";
                ?>
            </div>
        </div>
    </div>
...
function wptf_set_default_menu() {
...
function wptf_default_blog() {
   ...
   <div class="advs-box advs-box-side default-box <?php if (is_sticky()) echo "sticky-post"; ?>" data-anima="fade-left" data-trigger="hover">
    ...
    <ul class="pagination-sm pagination-grid pagination default-pagination">
        <li class="prev">
            <?php echo get_previous_posts_link(__("< Previous","gourmet")); ?>
        </li>
        <li class="next">
            <?php echo get_next_posts_link(__("Next >","gourmet"), $wp_query->max_num_pages ); ?>
        </li>
    </ul>
    <?php }
}
